...
validateSpot = [
  ...
  check('name').exists({ checkFalsy: true }).isLength({ min: 1, max: 49 })
    .withMessage('Name must be less than 50 characters'),
  check('description').exists({ checkFalsy: true }).notEmpty()
    .withMessage('Description is required'),
  check('price').exists({ checkFalsy: true }).isInt({ min: 0 })
...
validateQuery = [
  ...
  check('maxPrice').optional().isInt({ min: 0 })
...
router.get('/:spotId/bookings', requireAuth, async (req, res, next) => {
  ...
  if (spot.dataValues.ownerId === req.user.id) {
    ...
    const isOwner = await spot.getBookings({
